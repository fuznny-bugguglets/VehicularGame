...
public:
    ...
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Wheel Settings|Friction|Slope Effect", meta = (ClampMin = "0.0", ClampMax = "90.0", ToolTip = "Angle (degrees) from world up vector where friction stiffness reduction begins."))
    float FrictionReductionStartAngle = 30.0f;

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Wheel Settings|Friction|Slope Effect", meta = (ClampMin = "0.0", ClampMax = "90.0", ToolTip = "Angle (degrees) from world up vector where friction stiffness is maximally reduced. Must be >= FrictionReductionStartAngle."))
    float FrictionReductionMaxAngle = 60.0f;

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Wheel Settings|Friction|Slope Effect", meta = (ClampMin = "0.0", ClampMax = "1.0", ToolTip = "Minimum friction stiffness multiplier (0.0 to 1.0) applied when angle is at or beyond FrictionReductionMaxAngle."))
    float MinStiffnessMultiplierAtMaxAngle = 0.5f;
...
protected:
    ...
    UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Wheel State", meta = (ToolTip = "True if the wheel is currently in contact with a surface."))
    bool bIsGrounded = false;

    // Current compression distance of the suspension.
    UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Wheel State", meta = (ToolTip = "Current compression distance of the suspension in cm."))
    ...
    UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Wheel State|Slip", meta = (ToolTip = "Calculated forward slip of the tire."))
    float ForwardSlip = 0.0f;

    // Calculated sideways slip of the tire.
    UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Wheel State|Slip", meta = (ToolTip = "Calculated sideways slip of the tire."))
...
public:
    ...
    UFUNCTION(BlueprintCallable, Category = "Wheel Settings|Friction", meta = (ToolTip = "Call this after modifying ForwardFrictionCurve properties in Blueprint to update the internal curve representation."))
    void UpdateForwardFrictionCurveRuntime();

    UFUNCTION(BlueprintCallable, Category = "Wheel Settings|Friction", meta = (ToolTip = "Call this after modifying SidewaysFrictionCurve properties in Blueprint to update the internal curve representation."))
    void UpdateSidewaysFrictionCurveRuntime();

    UFUNCTION(BlueprintCallable, Category = "Wheel Settings|Friction", meta = (ToolTip = "Sets new properties for the forward friction curve and updates it."))
    void SetForwardFrictionProperties(const FWheelFrictionCurve& NewForwardFriction);

    UFUNCTION(BlueprintCallable, Category = "Wheel Settings|Friction", meta = (ToolTip = "Sets new properties for the sideways friction curve and updates it."))
    void SetSidewaysFrictionProperties(const FWheelFrictionCurve& NewSidewaysFriction);
...
private:
    ...
    // Updates the visual representation of the wheel.
    void UpdateWheelVisuals(float DeltaTime);

    // Calculates forward friction magnitude, including slope effects.
    float GetModifiedForwardFrictionMagnitude() const;

    // Calculates sideways friction magnitude, including slope effects.
    float GetModifiedSidewaysFrictionMagnitude() const;

    // Calculates friction stiffness multiplier based on slope.
    float GetCurrentSlopeStiffnessMultiplier() const;

    // Calculates interpolation speed for force smoothing.
