...
void AVehicularGameMode::SpawnWave()
{
	...
	CreateEnemies(VehicularGameState, PlayerPawn);
...
FVector AVehicularGameMode::CreateEnemies(AVehicularGameState* VehicularGameState, AActor* PlayerPawn)
{

	if (VehicularGameState == nullptr)
	{
		VehicularGameState = GetWorld()->GetGameState<AVehicularGameState>();
	}
	...
	if (EnemiesToSpawn.Num() > 0)
	{
		...
		if (PoolCount > 0)
		{
			...
			return FinalSpawnPoint->GetActorLocation();
		}
	}
